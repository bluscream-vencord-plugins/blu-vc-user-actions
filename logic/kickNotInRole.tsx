...
export const KickNotInRoleModule: PluginModule = {
    ...
    settings: kickNotInRoleSettings,
    ...
    onUserJoined: (channelId, userId) => {
        ...
        if (channel && settings.store.kickNotInRoleEnabled && settings.store.kickNotInRole) {
            checkKickNotInRole(userId, channelId, channel.guild_id);
        }
    }
};
...
export function checkKickNotInRole(userId: string, channelId: string, guildId: string) {
    const { settings } = require("../settings");
    const { GuildMemberStore } = require("@webpack/common");
    ...
    if (!member.roles.includes(requiredRole)) {
        log(`User ${userId} missing role ${requiredRole}, kicking from ${channelId}`);
